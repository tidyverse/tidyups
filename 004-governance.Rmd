...
This is part of a general movement to get better at defining and documenting our processes so that there's a clear path from package user to package developer and an understanding of how decisions are made.
...
The following sections define the four key roles (users, contributors, authors, and maintainers) and a few common processes.
...
The only requirement is to follow the [Code of Conduct](https://github.com/tidyverse/ggplot2/blob/master/CODE_OF_CONDUCT.md) and [contributing guidelines](https://github.com/tidyverse/ggplot2/blob/master/CONTRIBUTING.md).
...
To on-board a new author, the maintainer looks for rough consensus amongst authors by emailing them (since this needs to be done in privately).
...
> You can read about the rights and responsibilities of a package author at ..., but in short, being an author means that you'll be acknowledged in `Authors@R`, given write permission on GitHub, and help maintain the welcoming and inclusive tone for the project.
...
-   We are not currently considering a more open model where as soon as you get your first PR merged you're given write access (e.g. [trio](https://trio.readthedocs.io/en/latest/contributing.html#joining-the-team). Should we be?
