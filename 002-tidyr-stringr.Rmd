...
-   How do you specify the pieces?
    ...
    -   By length/position, e.g. `c(1, 3, 2)`/`c(1, 4, 6)`. split up string into piece of length 2 (more likely to go into rows).
    -   By a regular expression pattern, e.g. `","`, `"\s+"`.
    -   (Split by multiple separators in order; see question at end) Comes up in file names too.
    -   With regular expression "groups" formed by `()`,`"a(b)c"`.
    -   Might be worth sketching the process the leads to data in these various forms. `paste(x, "-", y, "|", z)`, `paste(x, collapse = "|")`, ...
...
Another option: `separate_wider()` + `separate_longer()` with three mutually exclusive arguments.
`at`, `by`, and `group` — although might call `position`, `delim`, `group`.Plus `str_separate_wider()` and `str_separate_longer()`.
Four new functions instead of 10.

`split = by_position(1, 2, 3)` / `split = by_width(1, 2, 3)`  
`split = by_separator(",", ignore.case = TRUE)`  
`split = by_regex_group("a(.)c")` (`by_group` is too short/ambiguous)

`split` defines how to break up the string into pieces.
There are five basic options which are further documented individually

-   `by_position(1, 2, 10)` splits into in two strings

```{r}
split_at = c(1, 2, 3) # is this widths or positions?
split_by = c(",")
split_group = c("a(.)c")
```

One other danger is that user might call `someotherfunction(x, by_position(1, 2, 3))`.
...
-   Another case: `"(.*),(.*)-(.*)"` — equivalent to `c(",", "-")` (if that worked, but it doesn't)
