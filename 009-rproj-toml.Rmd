...
designed as the single authoritative source of truth for both R
packages and projects (following cargo’s model), and specifies
how tools (like `rig`) read it, resolve dependencies against it, and can
generate a valid `DESCRIPTION` as a build artifact. `rproj.toml`'s main
goals are:
...
1. A `[tool.*]` namespace for other tools' config, analogous to
   `pyproject.toml`.

## Motivation

`DESCRIPTION` was designed decades ago around the needs of CRAN package
submission. Modern R project supports needs capabilities that are hard to
support in `DESCRIPTION`.

- **Flexible version constraints.**

  Only simple `>=`/`<=`/`==` comparisons against a single package are
  possible. There is no caret/tilde shorthand, and combining constraints is
  awkward.
...
build artifact, analogous to how `Cargo.toml` generates a build manifest
rather than being edited by hand alongside one.

Key design decisions:

- **Authoritative manifest.**

  `rproj.toml` is what a user edits. `DESCRIPTION` is generated from it at
  build/check time and should not be hand-edited once a project has an
  `rproj.toml`.
...
- **Two-axis optional dependencies.**
  ...
  website, dev), analogous to Python dependency groups. Both use the same
...
- **`[tool.<name>]` for other tools' config, distinct from `[config.<name>]`.**
  ...
  section under `[tool.*]` is free-form config owned by some other
  tool, the same role `[tool.ruff]`/`[tool.uv]` play in `pyproject.toml`.
  It lets one file replace several tool-specific TOML files in a
  project.
...
- **Round-trip fidelity.**
  ...
  to it in practice (poetry, uv, ...) avoid clobbering sections they
  don't own. We propose making it a requirement here: tools that write
...
1. `rig proj import` creates `rproj.toml` from a `DESCRIPRION` file.
...
1. `git`, `url` and `path` sources in `[dependencies]` and other
   dependency groups.
1. The resolution and installation of optional dependencies.
1. Custom repositories for dependencies. Only CRAN from PPM (Posit Package
   Manager) is supported currently.
1. Round-trip fidelity: `rig proj add` and similar commands currently
   rewrite `rproj.toml` in full, dropping comments, custom formatting,
   and any `[tool.*]` or other sections `rig` doesn't recognize. Fixing
   this is required for `[tool.*]` to be usable in practice.
