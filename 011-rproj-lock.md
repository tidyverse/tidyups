...
R has no lock file format. `renv.lock` is the closest thing, but it
records only versions and hashes, not enough to reproduce a solve: which
source or binary build was picked, or the exact URL and cache location
of every artifact.

This tidyup proposes `rproj.lock`, a TOML lock file that resolves an
`rproj.toml` manifest to a concrete, fully reproducible set of package
versions and builds for one `(R version, platform)` target, and
...
- **One file, an array of targets.**
  ...
  `[[targets]]` is keyed by `r_version` + `platform`. `rig proj lock`
  currently writes one target per invocation. `rig proj sync` installs
  the target matching the running machine. Multiple targets in one file
  (cross-platform lock files, mirroring `uv.lock`’s per-platform wheels)
  are anticipated but not implemented yet.
...
- **Every resolved package carries what installing it needs, nothing
  ...
  whose `groups` contains `"main"`. An earlier draft also carried fields
  `pak`’s own lockfile schema used (a manifest ref distinct from the
  package name, direct-vs-transitive, license, install args, system
  requirements); none of them are read by anything `rig` does with the
  lock file, so they were cut before the format shipped.
...
- **Metadata carries repository-specific provenance.**
  ...
  publishes that don’t fit the fixed schema, e.g. CRAN’s `RemoteHash`.
  This mirrors `DESCRIPTION`’s own `Remotes`-adjacent metadata fields
  rather than inventing a new provenance format.
...
| `metadata` | Open table for repository-specific provenance, e.g. CRAN’s `RemoteHash`. |
...
- Multiple `[[targets]]` per lock file (cross-platform solves in one
  file) is anticipated by the schema but not yet implemented. Should
  `rig proj lock` gain a flag to solve and append several targets at
  once, the way `uv lock` handles multi-platform resolution?
