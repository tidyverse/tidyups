...
`%in%` has historically been a common way to work around `NA` related issues because it handles the `NA`s for you by performing an "exact match" that only returns `TRUE` or `FALSE`, unlike `==` which propagates missing values.
...
Swapping `==` for `%in%` can sometimes make your `filter()` call clearer than using `is.na()`, but `exclude()` is still more readable.
